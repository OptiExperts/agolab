    ...
    document.addEventListener("click",function(event){
      ...
      if(els.classList.contains('apply-flits-credit')){
        ...
        that.getCartData(function (cart) {
          ...
          var params = "token=" +token+'&customer_hash='+window.flitsApp.credit_on_shipping_obj.customer_hash+ "&data=" + btoa(unescape(encodeURIComponent(JSON.stringify(cart))))+"&spent_rule_id="+spent_rule_id+"&shipping_price="+that.get_shipping_price();
          ajax.send(params);
        }, function () {
    ...
    ajax.onreadystatechange = function () {
      if (this.readyState == 4) {
        if (this.status == 200) {
          ...
          window.flitsApp.credit_on_shipping_obj.customer_hash = res.customer_hash;
